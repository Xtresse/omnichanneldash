  ...
  const rows = useMemo(
    () =>
      (orders || []).map((o) => ({
        ...
        _state: (o.state || "").trim(),
        _rep: o.rep || NO_REP,
        // Orders carry the ship-to ZIP as `shipZip` (see lib/windsor.js
        // orders map); zip5 normalizes ZIP+4 down to the 5-digit base.
        _zip: zip5(o.shipZip),
      })),
    [orders]
  );
            ...
            {breakdown.map((g) => {
              ...
              return (
                <tr key={g.key} className="border-b border-rule/60">
                  ...
                  <td className="py-2 pr-0 text-right text-ink font-semibold tabular-nums">{fmtPct(pct)}</td>
                </tr>
              );
            })}
            {!breakdown.length && (
          ...
          {breakdown.length > 0 && (
            <tfoot>
              <tr className="border-t-2 border-rule font-semibold">
                ...
                <td className="py-2 pr-0 text-right text-ink tabular-nums">100.0%</td>
              </tr>
            </tfoot>
          )}
        </table>
