...
export function planBoxForOrder(order) {
  ...
  const existingBox = lineItems.find((li) => BOX_SKUS.has(li.sku) && Number(li.quantity) > 0);

  if (existingBox) {
    if (existingBox.sku === DEFAULT_BOX_SKU) return null; // already correct
    return {
      action: "correct",
      removeSku: existingBox.sku,
      removeQty: Number(existingBox.quantity) || 1,
      addSku: DEFAULT_BOX_SKU,
      quantity: Number(existingBox.quantity) || 1,
      reason: "wrong-box-sku",
    };
  }
  ...
  return { action: "add", addSku: DEFAULT_BOX_SKU, quantity: 1, reason, gummyQty, hasSerum };
}
