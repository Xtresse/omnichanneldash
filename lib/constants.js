...
export const SKU_FAMILY = {
  ...
  "XTR-DTC-GM-01": "Gummies",      // DTC subscription — Hair Gummies 1 Month
  "XTR-DTC-GM-03": "Gummies",      // DTC subscription — Hair Gummies 3 Months
  ...
  "XTR-DTC-FR-01": "Serum",        // DTC serum single
  ...
  // ---- Legacy barcodes (historical orders, kept in sync with the rep
  // tracker's map so both dashboards classify these identically) ----
  "860011740117": "Serum",
  "860011740124": "Serum",
  "860011740131": "Sachets",
  "860011740148": "XVIE",
...
// SKU → family. Accepts an optional line-item TITLE used only as a safety net
// for lines whose SKU is blank or not yet in the map above — this mirrors the
// rep tracker's classifyLine() (lib/repData.js) so the two dashboards classify
// every line identically and tie to Shopify. Real catalog SKUs always resolve
// via the exact map / prefix rules; the title net never fires for them.
export function familyFor(sku, title) {
  if (sku && SKU_FAMILY[sku]) return SKU_FAMILY[sku];
  const u = String(sku || "").toUpperCase();
  // Pack-code guard: any *002CT* SKU is a 2ct sachet, never a gummy case —
  // catch it before the X-GN → Gummies prefix rule (matches the rep tracker).
  if (/002CT/.test(u)) return "Sachets";
  ...
  // Title fallback (blank/unknown SKUs only) — same order as the rep tracker:
  // xvie → serum → sachet → gummies. Sachet is checked before gummies because
  // the "Gummy On The Go" sachet title contains the word "Gummy".
  const t = String(title || "").toLowerCase();
  if (t) {
    if (/xvie/.test(t)) return "XVIE";
    if (/serum/.test(t)) return "Serum";
    if (/sachet|on the go/.test(t)) return "Sachets";
