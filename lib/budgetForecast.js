...
import { monthShapeFinish } from "./monthShapeForecast.js";
...
export function buildBudgetForecast(from, to, actualsByChannel = {}, opts = {}) {
  ...
  // MONTH-SHAPE forecast (final-week-lift): when the loaded window is true
  // month-to-date (starts on the 1st) and we have historical completion
  // fractions, project the finish as MTD ÷ (fraction of the month that has
  // historically landed by this day-of-month). Empirically ~3% accurate vs
  // the flat run-rate's ~14% miss on B2B. Falls back to the run-rate per
  // channel when history is missing. See lib/monthShapeForecast.js.
  const cf = opts.completionFractions || null;
  const isMtd = Boolean(f && parseYmd(from) && parseYmd(from).getTime() === monthStart(f).getTime());
  const msB2B = cf && isMtd ? monthShapeFinish(actualsByChannel.B2B, cf.B2B) : null;
  const msDTC = cf && isMtd ? monthShapeFinish(actualsByChannel.DTC, cf.DTC) : null;
  const usedMonthShape = msB2B != null || msDTC != null;
  ...
  out.forecastSource = haveSheetForecast ? "sheet" : usedMonthShape ? "monthshape" : "runrate";
  // Surface the month-shape calibration for the UI (fraction + sample months).
  out.monthShape = cf && isMtd
    ? {
        used: usedMonthShape,
        B2B: cf.B2B || null,
        DTC: cf.DTC || null,
      }
    : { used: false, B2B: null, DTC: null };
  ...
  for (const scenario of ["base", "stretch"]) {
    ...
    if (haveSheetForecast) {
      ...
      // Month-shape finish where available (true-MTD window + history), else
      // flat run-rate. Scenario-independent (both read the same actual pace).
      monthForecastB2B = msB2B != null ? msB2B : rr.B2B;
      monthForecastDTC = msDTC != null ? msDTC : rr.DTC;
    }
    const forecast = {
