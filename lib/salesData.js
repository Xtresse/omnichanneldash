// =============================================================================
// SALES DATA — dashboard aggregation layer (Shopify-only)
// =============================================================================
// Builds every dashboard slice (KPIs, channel/product/state breakdowns, rep
// performance, account aging, etc.) from canonical Shopify rows. The sole data
// source is the Shopify Admin API via the shared core (lib/xtresseCore.js →
// lib/shopify.js). Windsor.ai was removed entirely on 2026-06-08 — there is no
// connector fallback. (File was formerly lib/windsor.js; the "Windsor-shaped"
// row vocabulary is retained only as the internal field-name convention.)
// =============================================================================
...
// Pre-2026-04-01 orders without an explicit B2B/ADCS signal are treated as
// untagged B2B (the store's DTC channel ramped on this date).
...
// Data source: the Shopify Admin API ONLY, via the canonical shared core
// (lib/xtresseCore.js → lib/shopify.js). Windsor.ai has been removed
// entirely — there is no fallback connector. If Shopify creds aren't
// configured we throw rather than silently returning empty data.
function requireShopify() {
  if (!hasShopifyCreds()) {
    throw new Error(
      "Shopify credentials not configured. Set SHOPIFY_ADMIN_API_TOKEN " +
        "(or SHOPIFY_CLIENT_ID + SHOPIFY_CLIENT_SECRET) in the environment."
    );
  }
}

// Windowed sales rows for a preset or explicit {from, to}. Canonical
// Windsor-shaped rows (one per line item) — same shape every consumer
// already expects.
export async function fetchSalesRows({ preset, from, to } = {}) {
  requireShopify();
  return fetchShopifyRows({ preset, from, to });
}

// Full-history light pull (store opened 2024). The core's pull is
// quarterly-sharded — fast, and cached — so no separate light path is needed.
export async function fetchSalesAllTime() {
  requireShopify();
...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = []) {
  ...
  // 2026-05: restrict to REP-ATTRIBUTED B2B orders (repByOrder), matching
  // the canonical b2bNetSales definition. Previously this counted every
  // channel==='B2B' order — including untagged B2B-by-signal orders that
  // b2bNetSales deliberately routes to b2bUntaggedNetSales "so they don't
  // inflate the headline." That made the status bar's Total B2B (sum of
  // these focus families) drift ABOVE the B2B Net Sales KPI. With the
  // rep-attribution filter, each focus family is a strict subset of
  // b2bNetSales (sum of all line-allocated net for an order == o.net), so
  // the focus cards can never exceed the canonical B2B total the status
  // bar now ties to. Net (line-allocated) + gross (line-item) variants so
  // the status bar's Net/Gross toggle has both bases.
  const b2bFocusByFamily = {};
  const b2bFocusByFamilyGross = {};
  for (const fam of Object.keys(B2B_FOCUS_SKUS)) {
    ...
    for (const r of lineRows) {
      ...
      if (!repByOrder.has(r.order_id)) continue;
      if (allowed && !allowed.has(sku)) continue;
      const lineGross = numOrZero(r.line_item__price) * numOrZero(r.line_item__quantity);
      totalNet += lineGross * (netRatioByOrder.get(r.order_id) || 0);
      totalGross += lineGross;
    }
    b2bFocusByFamily[fam] = Math.round(totalNet);
    b2bFocusByFamilyGross[fam] = Math.round(totalGross);
  }
  ...
  const orders = orderRows
    .map((o) => {
      ...
      return {
        ...
        // Zip surfaced alongside state/rep so the Sales Explorer can
        // cross-filter on all three. Already captured in aggregateOrders
        // (used by the location-level new-account dedupe); just exposing it.
        zip: o.order_shipping_address_zip || null,
  ...
  return {
    ...
    topSKUs, productFamily, b2bFocusByFamily, b2bFocusByFamilyGross,
