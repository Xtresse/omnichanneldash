  ...
  for (const r of rawRows) {
    ...
    if (!isRefundRow) {
      ...
      if (sku) {
        ...
        if (familyFor(sku) === "Gummies") o.hasGummySku = true;
      }
    }
  }
...
export function buildDashboardData(rawRows, dateRange = {}) {
  ...
  for (const rep of Object.keys(REPS)) {
    repAgg[rep] = {
      ...
      net: 0,
      orders: 0,
      newAccounts: 0,         // chronological — first order from this customer with this rep within the loaded window
      firstOrderGummy: 0,     // tag-based — orders with both 'b2b' AND 'first order' tags AND a gummy SKU (matches leadership)
      lastOrderAt: null,
    };
  }
  ...
  // Count totals (used by reconciliation panel) of "first-order tagged" orders
  // — these are orders Shopify Flow tagged with both b2b + first order at
  // creation time, indicating a brand-new B2B account.
  ...
  for (const o of orderRows) {
    ...
    // Tag-based first-order check (matches leadership exactly)
    const lowerTags = parseOrderTags(o.order_tags).map((t) => String(t).toLowerCase());
    const isFirstOrderTagged =
      lowerTags.includes("b2b") && lowerTags.some((tg) => tg === "first order");
    if (isFirstOrderTagged && o.hasGummySku) {
      a.firstOrderGummy += 1;
