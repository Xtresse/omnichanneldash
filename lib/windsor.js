...
// Windsor.ai connectors API. The connector ID is part of the URL path.
// Reference: https://windsor.ai/api-documentation/
 ...
 *
 * @param {string} datePreset  e.g. "last_30d", "last_3m", "last_2years"
...
export async function fetchWindsorRows(datePreset = "last_2years") {
  ...
  // Field names verified against Windsor's Shopify schema
  // (https://windsor.ai/data-field/shopify/) — DO NOT change without re-verifying
  const fields = [
    "order_id",
    "order_created_at",
    "order_total_price",
  ...
  // Windsor's connector endpoint returns a flat array; some endpoints wrap in {data}
...
export function buildDashboardData(rawRows) {
  ...
  // First-touch by email within the channel.
  const firstSeen = new Map();
  for (const r of orderRows) {
    const email = (r.order_email || "").toLowerCase().trim();
    if (!email) continue;
    const ch = channelByOrder.get(r.order_id);
    const key = `${ch}|${email}`;
  ...
  // Windsor returns full state names ("California"); we display them as-is.
  const stateTotals = new Map();
  for (const r of orderRows) {
    const state = (r.order_shipping_address_province || "").trim();
    if (!state) continue;
    const ch = channelByOrder.get(r.order_id);
    const rev = Number(r.order_total_price) || 0;
  ...
  // Windsor's order_discount_codes mixes real promo codes (XVIE50, DIAMOND15)
  // with free-text reasons ("Approved by BC", "Custom discount"). Real codes
  // are contiguous (no spaces) and 3-30 chars; free-text reasons have spaces.
  ...
  for (const r of orderRows) {
    const loc = fulfillmentLocFor(r.order_shipping_address_province);
    const ch = channelByOrder.get(r.order_id);
