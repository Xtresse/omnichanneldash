...
export async function fetchWindsorRows({ preset, from, to } = {}) {
  ...
  const fields = [
    ...
    // overlay. Only `order_fulfillment_status` is reliably exposed by
    // Windsor's Shopify connector (verified via 400-on-other-names). The
    // tracking number and carrier aren't directly queryable, so we
    // surface fulfillment STATE (fulfilled / partial / null) — which
    // gives us "Shipped" vs "Pending" — and leave tracking numbers as a
    // future enhancement (would require a direct Shopify Admin API
    // call, which we can layer in once Sam confirms the priority).
