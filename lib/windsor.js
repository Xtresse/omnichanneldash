...
export async function fetchWindsorRows({ preset, from, to } = {}) {
  ...
  const fields = [
    ...
    // fulfillment for the order. order_fulfillment_status is the single
    // most useful field — values include "fulfilled", "partial", or null
    // when nothing has shipped. Tracking number/company/url come from
    // whichever fulfillment was last attached to the order in Shopify.
...
function deriveFulfillment(o) {
  ...
  const lineStatus = (o.line_fulfillment_status || "").toLowerCase();
  const shippedAt = o.fulfillment_created_at || null;
...
function aggregateOrders(rawRows) {
  ...
  for (const r of rawRows) {
    ...
    // Capture fulfillment fields from any row — keep the most recent
    // shipped_at since multi-fulfillment orders span multiple rows.
    const fStatus = r.order_fulfillment_status;
    if (fStatus && !o.fulfillment_status) o.fulfillment_status = String(fStatus);
    const lStatus = r.fulfillment_status;
    if (lStatus && !o.line_fulfillment_status) o.line_fulfillment_status = String(lStatus);
    const fAt = r.fulfillment_created_at;
    if (fAt && (!o.fulfillment_created_at || fAt > o.fulfillment_created_at)) {
      o.fulfillment_created_at = String(fAt);
      // Tracking attached to the same fulfillment row supersedes any older
      // tracking we captured.
      if (r.fulfillment_tracking_number) {
    ...
    } else if (!o.fulfillment_tracking_number && r.fulfillment_tracking_number) {
      // Fallback: capture tracking even when fulfillment_created_at is
      // missing (Windsor's rare null case).
      o.fulfillment_tracking_number = String(r.fulfillment_tracking_number);
      if (r.fulfillment_tracking_company) {
        o.fulfillment_tracking_company = String(r.fulfillment_tracking_company);
      }
...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = []) {
  ...
  for (const o of orderRows) {
    ...
    // Track the most-recent shipment we have on file for this rep so the
    // rep table can flag stale orders (no ship update in N days). Only
    // counts orders that actually have a fulfillment_created_at — a
    // pending-but-old order won't promote here, which is correct: it's
    // exactly the case Sam wants to see flagged.
    if (o.fulfillment_created_at) {
      const cur = a.lastShipment && a.lastShipment.shippedAt;
      if (!cur || o.fulfillment_created_at > cur) {
