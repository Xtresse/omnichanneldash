...
import { completionFractions } from "./monthShapeForecast.js";
...
// Real Shopify CompanyLocation-gid identity (2026-08-12, ported from
// Sales-Rep-Dashboards commit aa374c2 — "make it uniform across all
// dashboards" per Sam). custFromRow()/locationKeyFromRow() above key on
// customer_id + typed shipping-address text — vulnerable to the same
// three real bugs found and fixed tonight: an AP-contact email/customer
// record changing, a Company migrating to an aggregator, or an order
// skipping B2B attribution entirely all silently mint a new identity for
// the same physical door. This repo already tracks the resulting drift
// (see `chronologicalNewAccounts` / `reconciliation.newAccounts.delta`
// below) but never closed it — this is what closes it.
//
// Priority: order's own CompanyLocation gid -> ordering contact's own
// CompanyLocation role assignment -> exact-email match against a
// different, properly-attributed row -> legacy custFromRow() key. The
// emailToLocationKey map must be populated by the caller (module-level
// state here would leak across requests) — see its construction at each
// call site below.
function resolvedLocationKey(r, emailToLocationKey) {
  const cl = String(r.order_company_location_id || "").trim();
  if (cl) return `cl:${cl}`;
  const ccl = String(r.order_customer_company_location_id || "").trim();
  if (ccl) return `cl:${ccl}`;
  const emailKey = (r.order_email || "").toLowerCase().trim();
  if (emailKey && emailToLocationKey && emailToLocationKey.has(emailKey)) {
    return emailToLocationKey.get(emailKey);
  }
  return custFromRow(r);
}
// Registers a row's resolved key against its email, but ONLY when the row
// has a real CompanyLocation gid — so the map only ever hands out
// genuine, Shopify-confirmed identities (see Sales-Rep-Dashboards'
// emailToLocationKey for the full rationale, including why this is
// exact-email, not address-based: address-only matching was tried and
// reverted after it merged two unrelated clinics sharing one street
// address with different suite numbers).
function registerEmailLocation(r, key, emailToLocationKey) {
  const hasRealLocation = r.order_company_location_id || r.order_customer_company_location_id;
  const emailKey = (r.order_email || "").toLowerCase().trim();
  if (hasRealLocation && emailKey && !emailToLocationKey.has(emailKey)) {
    emailToLocationKey.set(emailKey, key);
  }
}
// Historical freeze: any order before this date keeps its OLD (tag-based
// or legacy-key) value exactly, so no past month's numbers move — only
// this cutoff month onward reflects the corrected identity logic. Full
// history is still scanned to seed correct detection for August-forward
// orders either way.
const HIST_FREEZE_YMD = "2026-08-01";
  ...
  for (const r of rawRows) {
    ...
    if (!byId.has(id)) {
      byId.set(id, {
        ...
        // Real Shopify CompanyLocation gids (see resolvedLocationKey below).
        order_company_location_id: r.order_company_location_id || null,
        order_customer_company_location_id: r.order_customer_company_location_id || null,
    ...
    if (!o.order_company_location_id && r.order_company_location_id)
      o.order_company_location_id = r.order_company_location_id;
    if (!o.order_customer_company_location_id && r.order_customer_company_location_id)
      o.order_customer_company_location_id = r.order_customer_company_location_id;
...
// logic"): the channel decision is now 100% the canonical
// classifyChannel() in lib/xtresseCore.js — the SAME function the DTC
// dashboard (and ops/finance trackers) run — so the two dashboards can
// never disagree on an order again. The signals this function used to
// carry locally (substring ADCS, ADCS-by-discount-code, rep/territory
// tag words) were folded INTO the core on this date. Omni adds exactly
// ONE thing on top: rep attribution (the core returns the channel; the
// rep name comes from the shared findRep) and the pre-DTC-launch date
// shim below.
function classifyOrderChannel(order) {
  const channel = classifyChannel({
    tagsRaw: order.order_tags,
    discountCodesRaw: order.order_discount_codes,
  });
  if (channel === "ADCS") return { channel: "ADCS", rep: null };
  if (channel === "B2B") {
    const rep = findRep(order.order_tags);
    return { channel: "B2B", rep: rep && rep !== "__EXCLUDE__" ? rep : null };
  }
  // channel === "DTC" — omni-only historical shim: before the DTC store
  // launched there was no DTC channel, so an untagged pre-2026-04-01
  // order is B2B-era unless it carries a DTC retail SKU (hasDtcSku).
  // This can never diverge from the DTC dashboard: every order that
  // dashboard counts contains a retail SKU, which rescues it here too.
  if (!order.hasDtcSku) {
...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = [], costs = null) {
  ...
  // Resolved (CompanyLocation-gid) identity infra — closes the drift this
  // repo's own reconciliation panel already surfaces (chronologicalNewAccounts
  // vs firstOrderGummyTotal delta). Scanned over `allTimeRows` (full history,
  // line-item grain), NOT the window-scoped `orderRows` — same reason
  // firstProductOrder above uses allTimeRows: an order at the start of the
  // loaded window is only genuinely "new" if there's truly no earlier order
  // for that identity, which a window-scoped scan can't see (every order in
  // the window would trivially look "first" with no prior history to
  // compare against). Two passes: first seed emailToLocationKey from every
  // row carrying a real CompanyLocation gid, then resolve every order's key
  // and its first-seen timestamp — tracked BOTH whole-account and
  // gummy-specific, mirroring the isAccountNewHist / isFirstGummyHist split
  // shipped in Sales-Rep-Dashboards (aa374c2): a "first order ever" tag is
  // not proof of "first gummy purchase" — an account's first order might
  // not even contain gummies. Gummy family match mirrors the o.hasGummySku
  // condition above (Gummies OR Sachets — a sachet purchase is still "tried
  // the gummies" for this signal).
  const emailToLocationKeyGlobal = new Map();
  for (const r of allTimeRows) {
    const ownKey = resolvedLocationKey(r, emailToLocationKeyGlobal);
    registerEmailLocation(r, ownKey, emailToLocationKeyGlobal);
  }
  const resolvedKeyByOrderId = new Map();
  const firstSeenLocationResolved = new Map();
  const firstSeenGummyLocationResolved = new Map();
  for (const r of allTimeRows) {
    const key = resolvedLocationKey(r, emailToLocationKeyGlobal);
    if (!resolvedKeyByOrderId.has(r.order_id)) resolvedKeyByOrderId.set(r.order_id, key);
    const t = new Date(r.order_created_at).getTime();
    if (!key || isNaN(t)) continue;
    if (!firstSeenLocationResolved.has(key) || t < firstSeenLocationResolved.get(key)) {
      firstSeenLocationResolved.set(key, t);
    }
    const fam = familyFor(r.line_item__sku);
    if (fam === "Gummies" || fam === "Sachets") {
      if (!firstSeenGummyLocationResolved.has(key) || t < firstSeenGummyLocationResolved.get(key)) {
        firstSeenGummyLocationResolved.set(key, t);
      }
    }
  }
  const ymdOf = (o) => (o.order_created_at || "").slice(0, 10);
  const isFirstOrderFixed = (o) => {
    const key = resolvedKeyByOrderId.get(o.order_id);
    const t = new Date(o.order_created_at).getTime();
    return !!key && !isNaN(t) && t === firstSeenLocationResolved.get(key);
  };
  const isFirstGummyFixed = (o) => {
    if (!(o.gummyLineGross > 0)) return false;
    const key = resolvedKeyByOrderId.get(o.order_id);
    const t = new Date(o.order_created_at).getTime();
    return !!key && !isNaN(t) && t === firstSeenGummyLocationResolved.get(key);
  };
  // Historical freeze (per Sam: "I told you not to touch historical except
  // for August") — every order before HIST_FREEZE_YMD keeps its old
  // tag-based value; August-forward orders get the corrected value. Both
  // maps are seeded from ALL orders (not just August+) since judging an
  // August order correctly requires full history under the fixed logic.
  const isFirstOrderTaggedById = new Map();
  const isFirstGummyTaggedById = new Map();
  for (const o of orderRows) {
    const legacyTag = parseOrderTags(o.order_tags).some(isFirstOrderTag);
    const ymd = ymdOf(o);
    isFirstOrderTaggedById.set(o.order_id, ymd < HIST_FREEZE_YMD ? legacyTag : isFirstOrderFixed(o));
    isFirstGummyTaggedById.set(o.order_id, ymd < HIST_FREEZE_YMD ? legacyTag : isFirstGummyFixed(o));
  }
  ...
  for (const o of orderRows) {
    ...
    // New-account gummy check: resolved-identity first-gummy-purchase
    // signal, freeze-gated (see isFirstGummyTaggedById construction above).
    // Historically this only checked the Shopify Flow 'first order' tag —
    // see Amy Pierre's Order #5389/#5392 (May 2026) report on why plain
    // tag presence, without a b2b-tag requirement, was already relaxed in
    // here; the resolved-identity layer on top closes the remaining gap
    // where tag automation fires on the wrong identity (AP-contact email
    // change, aggregator migration, attribution-skipped order).
    const isFirstOrderTagged = isFirstGummyTaggedById.get(o.order_id) === true;
  ...
  // isFirstOrderTaggedById / isFirstGummyTaggedById already built above
  // (resolved-identity + historical-freeze pass, right after firstByRepCust).
  ...
  for (const r of lineRows) {
    ...
    let isNew = false;
    if (fam === "Gummies") {
      // Resolved-identity gummy-specific first-purchase signal (freeze-gated,
      // see isFirstGummyTaggedById above). This matches the Excel "First Time
      // P Club" tab's Gummies logic, which pulls from the Financial Model
      // Report's First-Time Gummies? flag (column U) — historically driven
      // purely by Shopify's "first order" tag, now cross-checked against
      // resolved-location purchase history from August forward.
      isNew = isFirstGummyTaggedById.get(r.order_id) === true;
  ...
  for (const o of orderRows) {
    ...
    if (isFirstGummyTaggedById.get(o.order_id) === true && o.gummyLineGross > 0) {
  ...
  // ---- Lifetime reorder rate (customer / account-level) ----
  // The investor "repeat rate": the share of DISTINCT customers (DTC) /
  // accounts (B2B) that placed >=2 lifetime PAID orders. Computed from
  // ALL-TIME history (agingAgg = aggregateOrders(allTimeRows).filter(net>0)),
  // NOT the loaded window, so it stays stable regardless of the date picker —
  // a lifetime repeat rate is the figure investors ask about, and it must not
  // move when someone changes the range above. This is DELIBERATELY DISTINCT
  // from the order-level `repeatRate` series above (% of ORDERS in a bucket
  // that came from returning customers — RepeatRate.jsx), which is left alone.
  //
  // Channel comes from the shared classifyOrderChannel (agingChannelByOrder),
  // so it can never diverge from the rest of the dashboard. Identity:
  //   • DTC  → the customer (order_customer_id, which already falls back to
  //            email in the core) — a repeat CUSTOMER, not a repeat ship-to.
  //   • B2B  → the resolved Shopify CompanyLocation gid (location grain, via
  //            resolvedKeyByOrderId — NEVER the company/provider name, per the
  //            location-grain rule). B2B is restricted to rep-attributed orders
  //            (agingRepByOrder), matching the canonical b2bNetSales / Account
  //            Aging / "B2B Accounts" universe, all of which are rep-attributed.
  const dtcOrdersByCust = new Map();
  const b2bOrdersByAcct = new Map();
  for (const o of agingAgg) {
    const ch = agingChannelByOrder.get(o.order_id);
    if (ch === "DTC") {
      const key = String(o.order_customer_id || (o.order_email || "").toLowerCase().trim() || "");
      if (!key || key === "null") continue;
      dtcOrdersByCust.set(key, (dtcOrdersByCust.get(key) || 0) + 1);
    } else if (ch === "B2B") {
      if (!agingRepByOrder.get(o.order_id)) continue; // rep-attributed B2B only
      const key = resolvedKeyByOrderId.get(o.order_id) || resolvedLocationKey(o, emailToLocationKeyGlobal);
      if (!key) continue;
      b2bOrdersByAcct.set(key, (b2bOrdersByAcct.get(key) || 0) + 1);
    }
  }
  const reorderStatsOf = (m) => {
    let total = 0, repeat = 0;
    for (const c of m.values()) { total += 1; if (c >= 2) repeat += 1; }
    return { total, repeat, pct: total ? Math.round((repeat / total) * 1000) / 10 : 0 };
  };
  const dtcReorder = reorderStatsOf(dtcOrdersByCust);
  const b2bReorder = reorderStatsOf(b2bOrdersByAcct);
  const reorderRates = {
    basis: "lifetime",
    source: (allTimeRows && allTimeRows.length) ? "all-time" : "window",
    // DTC: distinct customers, and how many reordered (>=2 lifetime orders).
    dtc: { pct: dtcReorder.pct, customers: dtcReorder.total, repeat: dtcReorder.repeat },
    // B2B: distinct rep-attributed accounts (CompanyLocation grain), % that reordered.
    b2b: { pct: b2bReorder.pct, accounts: b2bReorder.total, repeat: b2bReorder.repeat },
  };
  ...
  try {
    // MONTH-SHAPE calibration: fraction of the month's net that historically
    // lands by this day-of-month, per channel, over the last 4 closed months
    // (from the all-time pull already in scope). Fed to the forecast so an MTD
    // window projects the finish empirically (final-week lift) instead of a
    // flat run-rate. Falls back to run-rate when history is thin. The fraction
    // is a timing SHAPE (channel-level via classifyChannel); applying it to the
    // rep-attributed b2bNetSales actual is sound because the shape is ~population
    // -independent. See lib/monthShapeForecast.js (Sam/Scott, 2026-07-25).
    let cf = null;
    // Only the month-to-date (window starts on the 1st) case uses month-shape;
    // skip the work entirely for last-7/30/90 warm windows.
    const isMtdWindow = /-01$/.test(String(windowFrom || ""));
    if (isMtdWindow) {
      try {
        cf = completionFractions(allTimeRows, windowTo, 4, ["B2B", "DTC"]);
      } catch {
        cf = null;
      }
    }
  ...
  return {
    ...
    // Lifetime (all-time) customer/account-level reorder rate per channel —
    // the investor "repeat rate", window-independent. See its computation above.
    reorderRates,
