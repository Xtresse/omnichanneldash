...
export async function fetchWindsorRows({ preset, from, to } = {}) {
  // Prefer direct Shopify when the app creds are configured; fall back to
  // Windsor otherwise. Same row shape either way.
  if (hasShopifyCreds()) return fetchShopifyRows({ preset, from, to });
  ...
  const fields = [
    ...
    // Address1/city/zip added 2026-05 for LOCATION-LEVEL new-account
    // dedupe. Each Shopify CompanyLocation under a parent Company is a
    // distinct purchasing entity; without these fields the per-rep
    // first-time customer sets collapsed multi-location chains to one
    // "new" event per company. See locationKeyFromRow() below.
...
export async function fetchWindsorAllTimeLight() {
  // Full history is only ~2.6k orders (store opened 2024-07), so the paginated
  // Shopify pull is a handful of pages — fast, and cached 5 min. Fully on
  // Shopify; Windsor is no longer used.
  if (hasShopifyCreds()) return fetchShopifyAllTimeLight();
  const apiKey = process.env.WINDSOR_API_KEY;
  if (!apiKey) throw new Error("WINDSOR_API_KEY not set");
  const account = process.env.WINDSOR_ACCOUNT || "ace1d0-26.myshopify.com";
  ...
  const fields = [
    ...
    // Address fields on the all-time history pull too — so the
    // location-level cust identifier matches between historical
    // first-product detection and current-window B2B aggregation.
    "order_shipping_address",
    "order_shipping_address_city",
...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = []) {
  ...
  // lifetime/trailing revenue + per-product split + order history. Built
  // from the loaded window's orders, so the Account Aging tab self-fetches
  // an all-history window (from store start) to capture dormant cohorts.
  // Account/rep/revenue all use the same order objects as the other tabs,
  // so it ties out. Excludes ADCS (per the canonical B2B convention).
  const acctKey = (o) => String(o.company || o.account || o.email || `o-${o.id}`).toLowerCase().trim();
  const acctMap = new Map();
  const nowTs = Date.now();
  for (const o of orders) {
    if (o.adcs || !o.date) continue;
    const key = acctKey(o);
    let a = acctMap.get(key);
    if (!a) {
      a = { name: o.company || o.account || o.email || "—", rep: o.rep || null, repLast: o.rep || null,
            firstOrder: o.date, lastOrder: o.date, lifetimeNet: 0, orders: 0,
            byProduct: { Gummies: 0, Serum: 0, XVIE: 0 }, history: [] };
      acctMap.set(key, a);
    }
    a.orders += 1;
    a.lifetimeNet += o.net || 0;
    if (o.date > a.lastOrder) { a.lastOrder = o.date; if (o.rep) a.repLast = o.rep; }
    if (o.date < a.firstOrder) a.firstOrder = o.date;
    if (!a.rep && o.rep) a.rep = o.rep;
    a.history.push({ date: o.date, name: o.name, net: Math.round(o.net || 0), channel: o.channel });
  }
  // Per-account per-family net from line rows.
  const acctByOrderId = new Map(orders.map((o) => [String(o.id), acctKey(o)]));
  for (const r of lineRows) {
    const key = acctByOrderId.get(String(r.order_id));
    if (!key) continue;
    ...
    const lg = numOrZero(r.line_item__price) * numOrZero(r.line_item__quantity);
    a.byProduct[fam] += lg * (netRatioByOrder.get(r.order_id) || 0);
  }
  ...
  return {
    ...
    orders, repPerformance, accountAging,
