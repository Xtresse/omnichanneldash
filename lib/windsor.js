...
const emptySlot = () => ({
  newUnits: 0, newDollars: 0, existingUnits: 0, existingDollars: 0,
});
...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = []) {
  ...
  const firstProductDate = new Map();
  ...
  for (const rep of Object.keys(REPS)) {
    repAgg[rep] = {
      rep, territory: REPS[rep][0], region: REPS[rep][1],
      net: 0, orders: 0,
      firstOrderGummy: 0,
  ...
  for (const o of orderRows) {
    ...
    // RELAXED: every order in this loop is already rep-tagged (verified by
    // findRep upstream), which means it IS B2B. So the 'first order' tag
    // alone is enough — the rep's manual tag is a deliberate signal.
    // Reported by Amy Pierre on Order #5389/#5392 (May 2026).
  ...
  for (const r of lineRows) {
    ...
    if (fam === "Gummies") {
      ...
      // Tag override: trust the rep's manual 'first order' tag.
