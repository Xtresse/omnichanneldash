...
export async function fetchWindsorRows({ preset, from, to } = {}) {
  ...
  const fields = [
    ...
    // Fulfillment / tracking — added 2026-05 for the rep-tracker shipping
    // overlay. Windsor exposes these directly from Shopify; they appear on
    // every fulfillment row and stay null until Shopify creates a
    // fulfillment for the order. order_fulfillment_status is the most
    // useful field — values include "fulfilled", "partial", or null when
    // nothing has shipped. Tracking number/company/url come from whichever
    // fulfillment was last attached to the order in Shopify.
    //
    // NOTE: bare `fulfillment_status` and `fulfillment_created_at` are NOT
    // exposed by Windsor's Shopify connector (only the order-level prefix
    // works). We approximate ship date from order_created_at when needed.
...
/**
 * Turn the raw Windsor fulfillment fields into a single object the UI can
 * consume directly:
 *   { status, label, shippedAt, daysInTransit, carrier, trackingNumber, trackingUrl }
 * Status is one of: "pending" | "shipped" | "in_transit" | "delivered" | "exception".
 *
 * Heuristic: Windsor's `order_fulfillment_status` covers Shopify's order-
 * level state (fulfilled / partial / null). It does NOT distinguish
 * shipped-but-not-delivered from delivered. Real delivery status only
 * lands when carriers report back (often via Shopify's tracking webhooks
 * → "delivered") and Windsor exposes that as `fulfillment_status` on the
 * fulfillment row. We treat `delivered` as terminal, and `success` /
 * `fulfilled` with a recent ship date as in-transit, and older
 * fulfillments without delivery confirmation as "shipped".
 */
function deriveFulfillment(o) {
  const orderStatus = (o.fulfillment_status || "").toLowerCase();
  const lineStatus = ""; // Windsor doesn't expose line-level status separately
  // Approximate ship date from the order created date when fulfilled —
  // Windsor doesn't expose fulfillment_created_at separately, but for
  // recently-shipped B2B orders the order date is within ~1-2 days of
  // ship which is good enough for a "days in transit" UX heuristic.
  const shippedAt = orderStatus === "fulfilled" || orderStatus === "partial"
    ? o.order_created_at || null
    : null;
  const trackingNumber = o.fulfillment_tracking_number || null;
  const trackingUrl = o.fulfillment_tracking_url || null;
  const carrier = o.fulfillment_tracking_company || null;

  let status = "pending";
  if (lineStatus === "delivered" || orderStatus === "delivered") {
    status = "delivered";
  } else if (lineStatus === "in_transit" || orderStatus === "in_transit") {
    status = "in_transit";
  } else if (
    ...
    // Generic "Shopify said it shipped". If we have a recent shipped_at
    // (≤ 7 days old) we still call it in_transit; older = just "shipped"
    // since there's no signal it actually arrived.
    if (shippedAt) {
      const ms = Date.now() - new Date(shippedAt).getTime();
      if (Number.isFinite(ms) && ms < 7 * 86400000) {
        status = "in_transit";
      } else {
        status = "shipped";
      }
  ...
  // Days in transit — null when no ship date yet. Counted in whole days
  // from fulfillment_created_at to today (or to delivered date if we had
  // one — Windsor rarely surfaces that, so we approximate).
  let daysInTransit = null;
  if (shippedAt) {
    const ms = Date.now() - new Date(shippedAt).getTime();
    if (Number.isFinite(ms) && ms >= 0) {
      daysInTransit = Math.floor(ms / 86400000);
    }
  }
  ...
  return {
    status,
    label: labelByStatus[status] || "Pending",
    shippedAt,
    daysInTransit,
    carrier,
    trackingNumber,
  ...
  for (const r of rawRows) {
    ...
    if (!byId.has(id)) {
      byId.set(id, {
        ...
        hasGummySku: false,
        gummyLineGross: 0,
        // Fulfillment / tracking — captured from whichever row has the
        // most-recent fulfillment_created_at. An order can have multiple
        // fulfillments (e.g. partial shipment + later remainder); the
        // single tracking number we surface is the most recent one.
        fulfillment_status: null,         // shopify order-level: fulfilled/partial/null
        line_fulfillment_status: null,    // line-item level: shipped/null
        fulfillment_created_at: null,     // most-recent ship date (ISO)
        fulfillment_tracking_number: null,
        fulfillment_tracking_company: null,
        fulfillment_tracking_url: null,
      });
    }
    const o = byId.get(id);

    // Capture fulfillment fields from any row. Windsor only exposes
    // order_fulfillment_status (order-level: fulfilled/partial/null) and
    // the tracking trio. We approximate shipped_at = order_created_at
    // when status is fulfilled — that's directionally right for the
    // "stale order" UX and gets refined if Windsor later adds the
    // fulfillment_created_at field.
    const fStatus = r.order_fulfillment_status;
    if (fStatus && !o.fulfillment_status) o.fulfillment_status = String(fStatus);
    if (!o.fulfillment_tracking_number && r.fulfillment_tracking_number) {
      o.fulfillment_tracking_number = String(r.fulfillment_tracking_number);
      if (r.fulfillment_tracking_company) {
        o.fulfillment_tracking_company = String(r.fulfillment_tracking_company);
      }
      if (r.fulfillment_tracking_url) {
        o.fulfillment_tracking_url = String(r.fulfillment_tracking_url);
      }
    }
...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = []) {
  ...
  for (const rep of Object.keys(REPS)) {
    repAgg[rep] = {
      ...
      lastOrderAt: null,
      // Most-recent shipment for the rep — captured in the loop below by
      // walking orderRows. Surfaces a "Last shipped X days ago" indicator
      // on the rep table so Sam can spot reps whose orders are stuck in
      // pending or who haven't had anything ship recently.
      lastShipment: null,
    };
  }
  ...
  for (const o of orderRows) {
    ...
    // Track the most-recent shipment we have on file for this rep. Windsor
    // exposes order-level fulfillment_status only — without a separate
    // shipped_at we use the order's created_at as the reference date for
    // "fulfilled" orders. That's good enough for "Last shipped X days ago"
    // — refines automatically once Windsor exposes the fulfillment date.
    if (o.fulfillment_status === "fulfilled" || o.fulfillment_status === "partial") {
      const cur = a.lastShipment && a.lastShipment.shippedAt;
      const shipDate = o.order_created_at;
      if (shipDate && (!cur || shipDate > cur)) {
        a.lastShipment = deriveFulfillment(o);
      }
    }
  }
  ...
  const orders = orderRows
    .map((o) => {
      ...
      return {
        ...
        // Fulfillment / tracking — null until Shopify ships the order.
        // Status is normalized to one of:
        //   "delivered" | "in_transit" | "shipped" | "pending" | "exception"
        // The UI maps this to friendly labels + colors. We compute days-
        // in-transit from fulfillment_created_at so the rep dashboard can
        // flag stale shipments without re-deriving from raw timestamps.
        fulfillment: deriveFulfillment(o),
      };
    })
    .sort((a, b) => {
