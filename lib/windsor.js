...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = [], costs = null) {
  ...
  // ---- Contribution margin: gross profit − merchant fees − fulfillment ----
  // Rates come from the Google Sheet actuals (lib/costsSheet): an effective fee
  // % (Σ fees ÷ Σ sales base) applied to window GROSS sales, and a blended
  // cost-per-order (Σ fulfillment ÷ Σ shipments) applied to window orders. Both
  // null when the sheet isn't wired — the dashboard then shows gross margin only.
  const feeRate = costs?.effective?.feeRate ?? null;
  const costPerOrder = costs?.effective?.costPerOrder ?? null;
  const merchantFees = feeRate != null ? Math.round(feeRate * kpis.totalGrossSales) : null;
  const fulfillment = costPerOrder != null ? Math.round(costPerOrder * kpis.totalOrders) : null;
  const haveOpCosts = merchantFees != null || fulfillment != null;
  const contribution = haveOpCosts
    ? Math.round(overallRev - overallCogs - (merchantFees || 0) - (fulfillment || 0))
    : null;
  ...
  grossMargin.contributionMarginPct =
    contribution != null && overallRev > 0
      ? Math.round((contribution / overallRev) * 1000) / 10
      : null;
  grossMargin.feeRatePct = feeRate != null ? Math.round(feeRate * 10000) / 100 : null;
  grossMargin.costPerOrder = costPerOrder != null ? Math.round(costPerOrder * 100) / 100 : null;
