...
// (xvieAcceleratorQualifies): during June 2026, a rep who sells
// >= 4 units of X-XVIE-2ML-006 OR >= 8 units of X-XVIE-2ML-003 —
// June units, EXACT net of returns — earns +2pp on their gummy
// (+sachets) commission rate. Unit counting mirrors
...
//
// NOTE: qualification here is by UNITS ONLY. Whether the +2pp bump is
...
function buildXvieAccelerator(allTimeRows) {
  ...
  // Progress toward qualification = best of the two SKU tracks. Either
  // track alone qualifies (it's an OR), so the leaderboard ranks by
  // whichever is furthest along; >100% just means cleared with room.
  const rows = Array.from(byRep, ([rep, s]) => {
    const progress = Math.max(
      s.units006 / XVIE_ACCEL.min006,
      s.units003 / XVIE_ACCEL.min003
    );
    const unitsToGo = Math.min(
      Math.max(0, XVIE_ACCEL.min006 - s.units006),
      Math.max(0, XVIE_ACCEL.min003 - s.units003)
    );
    return {
      rep,
    ...
    (a, b) =>
      ...
      b.units006 + b.units003 - (a.units006 + a.units003) ||
