...
//
// Canonical rules live in Sales-Rep-Dashboards/lib/compPlan.js
// (xvieAcceleratorQualifies / xvieAcceleratorVials): during June 2026 a
// rep who moves >= 24 total XVIE VIALS — COMBINABLE across the two promo
// SKUs (Sam, 2026-06-11): X-XVIE-2ML-006 case = 6 vials/unit,
// X-XVIE-2ML-003 starter = 3 vials/unit, so 4 cases alone, 8 starters
// alone, or any mix totalling 24 — earns +2pp on their gummy (+sachets)
// commission rate. Units are June only, EXACT net of returns, mirroring
// Sales-Rep-Dashboards/lib/repData.js: gross promo-SKU units on the
// order MINUS the EXACT returned units for that SKU (from the core's
// order_returned_qty_by_sku map, summed across all refunds on the
// order), clamped at 0 per order so a single order never contributes
// negative promo units. XVIE50 promo orders (50%-off code or tag) are
// excluded WHOLE-ORDER from unit counting (Sam, 2026-06-11; Bridget
// Selberg's #7506) — also mirrored from repData.js.
//
// Pinned to June 2026 and computed from the all-time pull, so the
// leaderboard reads the same regardless of the dashboard's selected
// window. Orders must be rep-attributed B2B (same classifyOrderChannel
// as the rest of the pipeline — ADCS and DTC never reach a rep) with
// net > 0 (cancelled / fully-refunded orders contribute nothing).
//
// NOTE: qualification here is by VIALS ONLY. Whether the +2pp bump is
// actually paid also depends on the rep's commission tier (window
// Tier-4 W-2 reps are excluded) — that lives in the Sales-Rep-Dashboards
// comp calc, not here.
const XVIE_ACCEL = {
  month: "2026-06",
  sku006: "X-XVIE-2ML-006",
  vialsPer006: 6, // case of 6 vials
  min006: 4, // single-SKU equivalent of the vial goal (display only)
  sku003: "X-XVIE-2ML-003",
  vialsPer003: 3, // starter pack of 3 vials
  min003: 8, // single-SKU equivalent of the vial goal (display only)
  vialsMin: 24,
};

// XVIE50 as a discount CODE or order TAG => the order's promo-SKU units
// don't count toward the accelerator (exact word, case-insensitive; same
// matcher as Sales-Rep-Dashboards/lib/xtresseCore.js hasXvie50). Both
// fields arrive as JSON-array strings on every row of the order.
function rowHasXvie50(r) {
  const has = (raw) => {
    if (!raw) return false;
    let arr;
    try {
    ...
    return (
      Array.isArray(arr) &&
      arr.some((c) => String(c).trim().toUpperCase() === "XVIE50")
    );
  };
  return has(r.order_discount_codes) || has(r.order_tags);
}

// Orders discounted more than this (order-level total, USD) are excluded
// WHOLE-ORDER from accelerator unit counting (Sam, 2026-06-18) — additive to
// the XVIE50 rule. Heavily-comped / ambassador orders (e.g. the "NEW
// AMBASSADOR" code, caught on Sonia Mace) must not credit the 24-vial goal.
// order_total_discounts is the order's total discount in USD (canonical core),
// repeated on every row; net dollars are unaffected, only the vial credit.
const XVIE_ACCEL_MAX_ORDER_DISCOUNT = 550;
function rowDiscountOver550(r) {
  return numOrZero(r.order_total_discounts) > XVIE_ACCEL_MAX_ORDER_DISCOUNT;
}

function buildXvieAccelerator(allTimeRows) {
  const empty = {
    month: XVIE_ACCEL.month,
    sku006: XVIE_ACCEL.sku006,
    vialsPer006: XVIE_ACCEL.vialsPer006,
    min006: XVIE_ACCEL.min006,
    sku003: XVIE_ACCEL.sku003,
    vialsPer003: XVIE_ACCEL.vialsPer003,
    min003: XVIE_ACCEL.min003,
    vialsMin: XVIE_ACCEL.vialsMin,
    rows: [],
    qualifiedCount: 0,
  };
  if (!Array.isArray(allTimeRows) || allTimeRows.length === 0) return empty;

  const promoRows = allTimeRows.filter(
    (r) => (r.order_created_at || "").slice(0, 7) === XVIE_ACCEL.month
  );
  if (promoRows.length === 0) return empty;

  // Rep attribution: same aggregation + channel classification as the
  // main pipeline. Only net>0, rep-attributed B2B orders count.
  const repById = new Map();
  for (const o of aggregateOrders(promoRows)) {
    if (o.net <= 0) continue;
    const { channel, rep } = classifyOrderChannel(o);
    if (channel === "B2B" && rep) repById.set(o.order_id, rep);
  }

  // Gross promo-SKU units per order, from sale line rows (refund offset
  // rows carry no line_item__sku). The exact returned-units map is
  // repeated on every row of the order — grab it once per order.
  const grossByOrder = new Map(); // order_id → { u006, u003 }
  const returnedRawByOrder = new Map(); // order_id → order_returned_qty_by_sku
  for (const r of promoRows) {
    if (!repById.has(r.order_id)) continue;
    // XVIE50 promo orders contribute ZERO accelerator units (whole-order).
    // ADDITIVE (Sam, 2026-06-18): orders discounted over $550 (the "NEW
    // AMBASSADOR" / ambassador-comp pattern) also contribute ZERO, whole-order.
    if (rowHasXvie50(r) || rowDiscountOver550(r)) continue;
    if (!returnedRawByOrder.has(r.order_id) && r.order_returned_qty_by_sku) {
      returnedRawByOrder.set(r.order_id, r.order_returned_qty_by_sku);
    }
    const sku = String(r.line_item__sku || "").trim();
    if (sku !== XVIE_ACCEL.sku006 && sku !== XVIE_ACCEL.sku003) continue;
    const qty = numOrZero(r.line_item__quantity);
    if (qty <= 0) continue;
    const g = grossByOrder.get(r.order_id) || { u006: 0, u003: 0 };
    if (sku === XVIE_ACCEL.sku006) g.u006 += qty;
    else g.u003 += qty;
    grossByOrder.set(r.order_id, g);
  }

  // Net units per rep = Σ per-order max(0, gross − exact returned per SKU).
  const byRep = new Map();
  for (const [orderId, g] of grossByOrder) {
    let returned = {};
    try {
      returned = JSON.parse(returnedRawByOrder.get(orderId) || "{}") || {};
    } catch {
      returned = {};
    }
    const u006 = Math.max(0, g.u006 - numOrZero(returned[XVIE_ACCEL.sku006]));
    const u003 = Math.max(0, g.u003 - numOrZero(returned[XVIE_ACCEL.sku003]));
    if (u006 <= 0 && u003 <= 0) continue;
    const rep = repById.get(orderId);
    const slot = byRep.get(rep) || { units006: 0, units003: 0, orders: 0 };
    slot.units006 += u006;
    slot.units003 += u003;
    slot.orders += 1;
    byRep.set(rep, slot);
  ...
  // Progress toward qualification = combined vials / 24. The two SKUs
  // COMBINE (case = 6 vials, starter = 3), so the leaderboard ranks by
  // total vials; >100% just means cleared with room.
  const rows = Array.from(byRep, ([rep, s]) => {
    const vials =
      s.units006 * XVIE_ACCEL.vialsPer006 + s.units003 * XVIE_ACCEL.vialsPer003;
    const progress = vials / XVIE_ACCEL.vialsMin;
    const vialsToGo = Math.max(0, XVIE_ACCEL.vialsMin - vials);
    return {
      rep,
      territory: (REPS[rep] || [])[0] || null,
      region: (REPS[rep] || [])[1] || null,
      units006: s.units006,
      units003: s.units003,
      vials,
      orders: s.orders,
      qualified: vials >= XVIE_ACCEL.vialsMin,
      progressPct: Math.round(progress * 100), // uncapped; UI caps the bar
      vialsToGo,
    };
  }).sort(
    (a, b) =>
      b.vials - a.vials ||
      b.units006 - a.units006 ||
...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = []) {
  ...
  // (Serum left this map 2026-06: first-serum is now tag-based — see
  // isSerumFirstTag in the rep loop below. Only XVIE + Sachets remain
  // chronological.)
  for (const r of allTimeRows) {
    ...
    if (fam !== "XVIE" && fam !== "Sachets") continue;
    const key = `${cust}|${fam}`;
  ...
  for (const r of allTimeRows) {
    ...
    const sku = (r.line_item__sku || "").trim();
    if (sku && familyFor(sku) === "Gummies") e.hasGummy = true;
  }
  ...
  // Tag-based first-SERUM check, same tolerant normalization. Two tags:
  //   - `First FR order` — Shopify Flow's automatic tag on an account's
  //     true first serum order. Flow keys off the account's real history,
  //     so a returning practice ordering under a different Shopify
  //     customer record (e.g. RISE Aesthetics, whose Feb serum sits under
  //     a separate customer than its May order) is correctly NOT tagged.
  //     The chronological first-ever-serum-date check this replaces split
  //     such practices across customer records and over-counted new-serum
  //     customers.
  //   - `First serum` — the manual rep equivalent when Flow doesn't fire
  //     (manually-keyed orders, draft conversions — same gap as the
  //     relaxed `first order` check below). Reported by Megan Gilbert on
  //     Order #7966 (Apres Aesthetics, June 2026).
  // Mirrors hasSerumFirstTag() in xtresse-leadershipdash/lib/windsor.js
  // (honored there since leadershipdash PR #10, June 2026) so the two
  // dashboards agree. No equivalent tag exists for XVIE, so XVIE (and
  // Sachets) stay chronological.
  const isSerumFirstTag = (t) => {
    const norm = t.replace(/\s+/g, " ").trim().toLowerCase();
    return norm === "first fr order" || norm === "first serum";
  };
  ...
  //   - Gummies: order is tagged `first order` (Shopify Flow / rep)
  //   - Serum: order is tagged `First FR order` (Shopify Flow) or
  //     `First serum` (rep) — see isSerumFirstTag above
  //   - XVIE / Sachets: customer's first-ever order of that product
  //     (from the all-time history pull) is the current order
  //
  ...
  // Pre-compute per-order: is this order tagged 'first order' (gummies)
  // or 'First FR order' / 'First serum' (serum)?
  // First-order is relaxed from b2b+first-order to just first-order — see
  // the comment in the gummy-count block above for context.
  const isFirstOrderTaggedById = new Map();
  const isSerumFirstTaggedById = new Map();
  for (const o of orderRows) {
    const tags = parseOrderTags(o.order_tags);
    isFirstOrderTaggedById.set(o.order_id, tags.some(isFirstOrderTag));
    isSerumFirstTaggedById.set(o.order_id, tags.some(isSerumFirstTag));
  }
  ...
  for (const r of lineRows) {
    ...
    if (fam === "Gummies") {
      ...
      // Tag-based (2026-06) — `First FR order` / `First serum`, see
      // isSerumFirstTag above. Replaces the chronological first-ever-
      // serum check, which over-counted practices ordering under
      // multiple Shopify customer records (RISE Aesthetics case).
      // Matches xtresse-leadershipdash.
      isNew = isSerumFirstTaggedById.get(r.order_id) === true;
    } else if (cust && cust !== "null") {
      // History-based, per-order (XVIE / Sachets — no first-purchase tag
      // exists for these families). The Excel/Financial Model Report's
      // First-Time flag is set on the SPECIFIC order
  ...
  for (const r of lineRows) {
    ...
    const lineCogs = cogsPerUnit(sku, familyFor) * netUnits;
  ...
  const grossMargin = {
    placeholder: COGS_IS_PLACEHOLDER,
    note: COGS_PLACEHOLDER_NOTE,
  ...
  return {
    ...
    // June-2026 XVIE accelerator leaderboard — pinned to June from the
    // all-time pull, independent of the selected window (empty in compare
    // snapshots, which pass allTimeRows = []).
    xvieAccelerator: buildXvieAccelerator(allTimeRows),
