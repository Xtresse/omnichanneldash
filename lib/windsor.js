...
export function buildDashboardData(rawRows, dateRange = {}, allTimeRows = []) {
  ...
  for (const r of lineRows) {
    ...
    if (fam === "Gummies") {
      ...
      // History-based ONLY — first-ever date for this customer × family
      // must fall inside the loaded window. Matches the Excel/Financial
      // Model Report's First-Time XVIE/Serum/Sachets flags (columns V/W/X),
      // which are purely "has this customer ever bought this family
      // before?" — no tag override.
      //
      // 2026-05: removed the previous "first order tag" defensive
      // fallback. Sam confirmed XVIE/Serum/Sachets first-time should be
      // *purely* history-based — a customer who has bought Serum before
      // is "Returning Serum" even if a specific order is tagged
      // 'first order' (the tag is about the *account relationship*, not
