...
//
// The all-time window is the largest payload (~4.9 MB raw → ~0.89 MB after
// gzip+base64 today). That already fits, but headroom is thin and the store
// keeps growing, so the ceiling is 2 MB. This is safe: if a payload ever
// exceeds the KV plan's real request limit, kv.set throws, setCachedData
// catches it and returns { ok:false } (and the warmer surfaces the byte size),
// so the worst case is "served live, not cached" — never a crash or bad data.
const MAX_CACHE_BYTES = 2_000_000; // ~2 MB; all-time ≈ 0.89 MB compressed today
