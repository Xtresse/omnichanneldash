...
// Every write carries a REAL Redis expiry. Freshness is still governed by the
// embedded `at` timestamp — this is purely storage hygiene.
//
// 2026-08-08: without it, nothing ever expired. Each distinct window a user
// picked minted a permanent `dash:v4:{...}` key (plus `#part` keys once
// chunking landed), and the Upstash DB filled to its 256 MiB quota — at which
// point EVERY write started failing with "DB capacity quota exceeded" and the
// warm cron silently warmed 0/13. Two days of expiry is far longer than any
// entry stays useful and bounds total storage to recent keys.
const DEFAULT_TTL_SEC = 48 * 60 * 60;

async function writeRaw(key, value, ttlSec = DEFAULT_TTL_SEC) {
  if (HAS_KV) {
    const kv = await kvClient();
