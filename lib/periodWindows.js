...
/**
 * President's Club program-year-to-date: Feb 1 – today, NOT the calendar
 * year. The program runs Feb 1 – Jan 31 (2026-2027 program year started
 * 2026-02-01), so this is intentionally separate from the generic "ytd"
 * case above — do not fold the two together, and do not use this for any
 * section other than President's Club. If today falls in January the
 * program year began Feb 1 of the PRIOR calendar year.
 */
export function presidentsClubYtdRange(today = shopTodayD()) {
  const t = today;
  const startYear = t.getMonth() >= 1 ? t.getFullYear() : t.getFullYear() - 1;
  return [ymd(new Date(startYear, 1, 1)), ymd(t)];
}
...
/**
 * Cache key for a precomputed rep × day heat map window. Includes the period
 * SHAPE because during the first month of a quarter (or Q1 vs YTD) two toggles
 * produce byte-identical [from,to] windows but DIFFERENT prior-trend
 * comparisons — keying on from/to alone made them collide on one entry.
 */
export const heatmapCacheKey = (from, to, shape = "auto") =>
  `hm:v2:${JSON.stringify({ shape, from, to })}`;
