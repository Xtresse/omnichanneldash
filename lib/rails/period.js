 ...
 * Mirrors lib/salesData.js#computeCompareWindow but exposed via the rail
...
export function compareWindow({ from, to }, mode = "prior") {
  ...
  const priorTo = new Date(fromD.getTime() - DAY_MS);
  const priorFrom = new Date(priorTo.getTime() - (toD.getTime() - fromD.getTime()));
  return { from: ymd(priorFrom), to: ymd(priorTo), mode: "prior" };
}
