// Dataset loader — wraps the Shopify sales + budget fetchers behind a single
// memoized entry point. Every rail gets its data through here, which means
// one Shopify pull per (period, request) instead of N pulls when Claude
// chains tool calls.
//
...
export async function loadPeriod(ctx, period) {
  ...
  const promise = (async () => {
    const [rows, allTime] = await Promise.all([
      fetchSalesRows({ from: period.from, to: period.to }),
      fetchSalesAllTime(),
    ]);
...
export async function loadCompare(ctx, period, mode = "prior") {
  ...
  if (cache.has(key)) return cache.get(key);
  const promise = (async () => {
    const rows = await fetchSalesRows({ from: win.from, to: win.to });
