...
import { buildScenarioSnapshot, windowStartFor } from "./scenario.js";
...
const RAILS = {
  ...
  run_scenario: {
    ...
    async run(
      ...
      const period = { from: ymd(fromDate), to: ymd(toDate), preset: null };
      const data = await loadPeriod(ctx, period);
      // Resolve horizon up front so a custom request without endDate
      // fails with a clean error before we hit the math layer.
      const resolvedHorizon = horizon === "custom" ? "custom" : horizon;
      const effectiveEndDate = horizon === "custom" ? endDate : undefined;
      const snap = buildScenarioSnapshot({
        dashboardData: data,
        windowDates: period,
        assumptions: {
