...
function ordersToRows(orders) {
  ...
  for (const order of orders) {
    ...
    const orderFields = {
      ...
      // Reduction from refunds + cancellations + order edits, so
      // net (= subtotal - this) equals Shopify's current subtotal.
      order_refunds_subtotal: String(Math.max(0,
        (parseFloat(money(order.subtotalPriceSet)) || 0) -
        (parseFloat(money(order.currentSubtotalPriceSet)) || 0))),
