...
export const SKU_FAMILY = {
  ...
  "X-FRC-30ML-CASE": "Serum", "X-FRC-30ML-001": "Serum", "XTR-DTC-FR-01": "Serum",
  "X-XVIE-2ML-006": "XVIE", "X-XVIE-2ML-001": "XVIE", "X-XVIE-2ML-003": "XVIE", "X-XVIE-003": "XVIE",
  "XTR-DTC-GM-01": "Gummies", "XTR-DTC-GM-03": "Gummies",
  // Legacy barcodes (historical orders)
  "860011740117": "Serum", "860011740124": "Serum", "860011740131": "Sachets", "860011740148": "XVIE",
...
// SKU → family. Optional line-item TITLE is a safety-net fallback for blank or
// unknown SKUs, mirroring omnichanneldash's lib/constants.js familyFor so every
// dashboard classifies identically and ties to Shopify. Real catalog SKUs always
// resolve via the exact map / prefix rules; the title net never fires for them.
export function familyFor(sku, title) {
  if (sku && SKU_FAMILY[sku]) return SKU_FAMILY[sku];
  const u = String(sku || "").toUpperCase();
  // Pack-code guard: any *002CT* SKU is a 2ct sachet, before X-GN → Gummies.
  ...
  // Title fallback (blank/unknown SKU) — omni order: xvie → serum → sachet → gummies.
