 ...
 *
 * Used for DTC and ADCS always (no rep-level breakdown makes sense for
 * those). Its B2B rows are a company-level FALLBACK: read only for months
 * where "Rep Targets" has no per-rep detail at all. A month with real rep
 * data (built via the rep engine, e.g. the current month) keeps that as
 * authoritative and is never overwritten by the coarser company-level
 * number — see repMonths in loadBudgetAndGoals.
 *
 ...
 */
function parseRepTargetsTab(rows, company, rep, reps, repMonths) {
  ...
  for (const row of rows.slice(1)) {
    ...
    if (TIER_BASIS_COLS.some(({ tier, basis }, i) => colIx[i] >= 0 && num(row[colIx[i]]))) repMonths.add(month);
 ...
 * Month). DTC/ADCS always come from here. B2B rows here are a FALLBACK,
 * company-level only (no per-rep detail) — used only for months where
 * `repMonths` has no Rep Targets data at all, so a month with real per-rep
 * detail (e.g. the current month) keeps that as authoritative and is never
 * overwritten by the coarser company-level number.
 * Mutates `company`.
 */
function parseCompanyTargetsTab(rows, company, repMonths) {
  if (!rows || rows.length < 2) return;
  const { entIx, prodIx, monthIx, colIx } = findCols(rows[0].map(norm));
  if (entIx < 0 || prodIx < 0 || monthIx < 0) return;
  ...
  for (const row of rows.slice(1)) {
    ...
    const product = canonProduct(row[prodIx]);
    const month = rowMonth(row, monthIx);
    if (!ch || !product || !month) continue;
    if (ch === "B2B" && repMonths.has(month)) continue; // rep-level detail wins
...
export async function loadBudgetAndGoals() {
  ...
  const rep = {};
  const reps = new Set();
  const repMonths = new Set(); // months with real per-rep B2B detail — takes priority over the Company Targets fallback
  parseRepTargetsTab(repRows, company, rep, reps, repMonths);
  parseCompanyTargetsTab(coRows, company, repMonths);
