 ...
 *                        [optional 4th col: forecast_net_revenue_usd]
 *   - "Rep Goals"      — columns: rep_name | product_family | month | goal_net_revenue_usd
 *
 * FORECAST (added 2026-06): if the "Company Budget" tab carries an optional
 * forecast column (header matches "forecast"), it is read into `forecast`
 * (same shape as `budget`). When absent, `forecast` is an empty {} and the
 * dashboard computes a run-rate forecast instead (lib/budgetForecast.js).
 * Dropping a forecast column into the sheet later is a value-only change —
 * no shape change here.
 *
 * Months are stored as ISO YYYY-MM strings. Product families are
 * "Gummies", "Serum", "XVIE", "Sachets".
 *
 * ───────────────────────────────────────────────────────────────
...
// Note: "Sachets" plural to match the dashboard's productFamily key
// from lib/windsor.js, which emits family="Sachets".
 ...
 *     forecast: { [product]: { [month]: $ } },  // empty {} when no sheet column
...
function shapeFromRaw(budgetRaw, goalsRaw, mode) {
  ...
  // Forecast lives in an OPTIONAL 4th column on the Company Budget tab. Same
  // { product: { month: $ } } shape as budget. Empty when the column is
  // absent — the dashboard's run-rate fallback (lib/budgetForecast.js) takes
  // over in that case, so adding the column later is a value-only change.
  const forecast = { Gummies: {}, Serum: {}, XVIE: {}, Sachets: {} };
  if (budgetRaw.length > 1) {
    const h = budgetRaw[0];
    const pi = findCol(h, "product");
    const mi = findCol(h, "month");
    const ai = findCol(h, "budget", "amount", "revenue", "usd");
    const fi = findCol(h, "forecast", "projection", "projected");
    for (let r = 1; r < budgetRaw.length; r++) {
      ...
      // Only populate forecast when a real forecast column exists AND isn't
      // the same column we already read as budget (findCol "usd"/"revenue"
      // could otherwise alias). A blank cell yields 0 and is skipped so the
      // run-rate fallback still kicks in for months left empty.
      if (p && m && fi >= 0 && fi !== ai) {
        const fv = num(row[fi]);
        if (fv) forecast[p][m] = (forecast[p][m] || 0) + fv;
      }
    }
  }
  ...
  if (goalsRaw.length > 1) {
    ...
    for (let r = 1; r < goalsRaw.length; r++) {
      ...
      if (!rep || !p || !m) continue;
      reps.add(rep);
      repGoals[rep] = repGoals[rep] || { Gummies: {}, Serum: {}, XVIE: {}, Sachets: {} };
      repGoals[rep][p][m] = (repGoals[rep][p][m] || 0) + v;
    }
  }

  return { mode, budget, forecast, repGoals, reps: [...reps].sort() };
}
...
function makeStubData() {
  ...
  // Forecast stub mirrors budget shape (all empty — run-rate fallback fills in).
  const forecast = { Gummies: {}, Serum: {}, XVIE: {}, Sachets: {} };
  return { mode: "stub", budget, forecast, repGoals: {}, reps: [] };
}
